...
export function startSync(initial){
 let active=initial,pending=false,busy=false;
 ...
 function status(error=false){
  const stale=Date.now()-Date.parse(active.importedAt)>3*60*60*1000;
  gate(!pending&&!stale);
  box.textContent=pending?'新しい案件データがあります。再読み込みして条件を確認してください。':stale?`最終同期 ${stamp()} · 3時間以上更新されていません。安全のため共有を停止中。`:error?`更新確認に失敗しました。最終同期 ${stamp()}。再確認できます。`:`1時間ごとに自動同期 · 最終同期 ${stamp()}（日本時間）`;
 ...
 async function check(){
  ...
  try{
   ...
   if(!next.contentHash||!next.records||!Date.parse(next.importedAt))throw Error('invalid');
 ...
 status();setInterval(check,60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)check()});
}
